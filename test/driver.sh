...
(cd "$tmp" || exit; "$OLDPWD"/"$chibicc" -c out.c)
[ -f "$tmp"/out.o ]
check 'default output file'

(cd "$tmp" || exit; "$OLDPWD"/"$chibicc" -c -S out.c)
[ -f "$tmp"/out.s ]
check 'default output file'
...
(cd "$tmp" || exit; "$OLDPWD"/"$chibicc" -c "$tmp"/foo.c "$tmp"/bar.c)
...
echo 'int x;' > "$tmp"/foo.c
echo 'int y;' > "$tmp"/bar.c
(cd "$tmp" || exit; "$OLDPWD"/"$chibicc" -c -S "$tmp"/foo.c "$tmp"/bar.c)
...
(cd "$tmp" || exit; "$OLDPWD"/"$chibicc" foo.c)
[ -f "$tmp"/a.out ]
check a.out
...
(cd "$tmp" || exit; "$OLDPWD"/"$chibicc" -c -MD -I. md2.c md3.c)
